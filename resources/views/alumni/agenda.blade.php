...
@section('content')
<!-- Hero Section -->
<section class="relative h-[400px] flex items-center justify-center text-center text-white bg-cover bg-center" style="background-image: url('{{ asset('images/Branda.jpg') }}');">
...
<div class="max-w-4xl mx-auto px-6 py-8">
    @if (session('success'))
        <div class="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded-xl mb-6 animate-pulse">
            {{ session('success') }}
        </div>
    @endif
            ...
            <div class="flex items-center gap-3 flex-1">
                ...
                <div class="flex-1">
                    ...
                    <span class="text-sm text-gray-500 flex items-center gap-1">
                        ...
                        {{ \Carbon\Carbon::parse($agenda->tanggal_mulai)->isoFormat('D MMM YYYY') }} @if($agenda->tanggal_selesai) - {{ \Carbon\Carbon::parse($agenda->tanggal_selesai)->isoFormat('D MMM YYYY') }} @endif
                    </span>
                </div>
            </div>
            <div class="arrow-wrapper p-2">
                <svg class="arrow-down w-6 h-6 text-gray-500 transition-transform duration-300 cursor-pointer" onclick="event.stopPropagation(); toggleAgenda(this.closest('.agenda-card'))" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7 -7"></path>
                </svg>
            </div>
        </div>
        <div class="content max-h-0 overflow-hidden bg-gradient-to-b from-gray-50 to-white p-8 border-t border-gray-200 transition-all duration-500 ease-in-out">
            <p class="text-green-700 font-semibold mb-6 flex items-center gap-2">
                <i class="bi bi-person-fill"></i> Kantor Alumni IAIT
            </p>
            <div class="bg-gradient-to-r from-green-50 to-emerald-50 p-8 rounded-2xl mb-8 text-sm text-gray-700 space-y-3 border border-green-100">
                <p><strong class="text-green-800">Lokasi:</strong> {{ $agenda->lokasi ?? 'IAIT Tasikmalaya' }}</p>
                <p><strong class="text-green-800">Waktu:</strong> {{ \Carbon\Carbon::parse($agenda->tanggal_mulai)->format('l, d F Y') }} @if($agenda->tanggal_selesai) (s/d {{ \Carbon\Carbon::parse($agenda->tanggal_selesai)->format('d F Y') }}) @endif</p>
            </div>
            <img src="{{ $agenda->gambar ? asset('storage/' . $agenda->gambar) : asset('images/L.jpeg') }}" alt="{{ $agenda->judul }}" class="rounded-2xl w-full max-w-3xl mx-auto block mb-8 shadow-xl object-cover">
...
function toggleAgenda(card) {
    ...
    if (isOpen) {
        content.classList.remove('open', 'p-8');
        ...
        document.querySelectorAll('.agenda-card').forEach(c => {
            if (c !== card) {
                const con = c.querySelector('.content');
                const arr = c.querySelector('.arrow-down');
                con.classList.remove('open', 'p-8');
                con.classList.add('max-h-0');
        ...
        content.classList.add('open', 'p-8');
